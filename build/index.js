...
async function refreshAccessToken() {
    ...
    try {
        const response = await axios.post(`${SPOTIFY_AUTH_BASE}/api/token`, querystring.stringify({
        ...
        if (isInvalidGrant(error)) {
            ...
            discardStoredTokens();
            throw new AuthenticationError("Spotify refresh token expired or revoked (invalid_grant). Stored credentials were discarded — re-authenticate with the auth-spotify tool (or set a fresh SPOTIFY_REFRESH_TOKEN in headless/remote environments).");
        }
 ...
 */
async function spotifyApiRequest(endpoint, method = "GET", data = null) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "search-spotify") {
            ...
            const results = await spotifyApiRequest(`/search?${querystring.stringify({
                q: query,
                type,
                limit,
            })}`);
...
URL: ${playlist.external_urls.spotify}`,
        ...
        if (name === "get-playlist-tracks") {
            ...
            const result = await spotifyApiRequest(`/playlists/${encodeURIComponent(playlistId)}/items?${params}`);
...
---`;
        ...
        if (name === "delete-playlist") {
            ...
            await spotifyApiRequest(`/playlists/${encodeURIComponent(playlistId)}/followers`, "DELETE");
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === "remove-tracks-from-playlist") {
            ...
            await spotifyApiRequest(`/playlists/${encodeURIComponent(playlistId)}/items`, "DELETE", { items });
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === "update-playlist") {
            ...
            await spotifyApiRequest(`/playlists/${encodeURIComponent(playlistId)}`, "PUT", body);
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === "get-playlist-cover") {
            ...
            const formatted = images
                .map((img) => `${img.width && img.height ? `Size: ${img.width}x${img.height}\n` : ""}URL: ${img.url}`)
                .join("\n---\n");
            return {
                content: [
        ...
        if (name === "get-recently-played") {
            ...
            const formatted = result.items
                .map((item) => `Track: ${item.track.name}
...
---`)
        ...
        if (name === "reorder-playlist-tracks") {
            ...
            await spotifyApiRequest(`/playlists/${encodeURIComponent(playlistId)}/items`, "PUT", {
